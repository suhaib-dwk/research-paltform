...
$stmt = $conn->prepare(
    "SELECT id, citation_style, notes, file_name, file_size, file_ext,
            status, result_notes, result_file_name, result_file_path, result_file_size, created_at
     FROM proofreading_requests WHERE user_id = ? ORDER BY created_at DESC"
);
...
while ($row = $res->fetch_assoc()) {
    $requests[] = [
        ...
        'meta_ar' => $row['citation_style'] ? ('نمط التوثيق: ' . $row['citation_style']) : '',
        'meta_en' => $row['citation_style'] ? ('Citation style: ' . $row['citation_style']) : '',
