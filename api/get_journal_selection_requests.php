...
$stmt = $conn->prepare(
    "SELECT id, research_field, priority, notes, file_name, file_size, file_ext,
            status, result_notes, result_file_name, result_file_path, result_file_size, created_at
     FROM journal_selection_requests WHERE user_id = ? ORDER BY created_at DESC"
);
...
while ($row = $res->fetch_assoc()) {
    $requests[] = [
        ...
        'meta_ar' => $row['research_field'],
        'meta_en' => $row['research_field'],
