<?php
// =====================================================================
// submit_publication.php — تقديم طلب نشر جديد (خدمة publication)
// =====================================================================
error_reporting(0);
...
$targetJournal = trim($_POST['target_journal'] ?? '');
...
$fileName = 'publication_' . $userId . '_' . time() . '_' . preg_replace('/[^a-zA-Z0-9._-]/', '_', $originalName);
...
$stmt = $conn->prepare(
    "INSERT INTO publication_requests
        (user_id, target_journal, notes, file_name, file_path, file_size, file_ext, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')"
);
$stmt->bind_param('issssis', $userId, $targetJournal, $notes, $fileName, $filePath, $fileSize, $fileExt);
...
log_activity($conn, $userId, 'service_request_submit', 'publication', 'طلب نشر جديد', 'New publication request', 'publication_requests', $newId);
