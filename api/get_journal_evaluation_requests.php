...
$stmt = $conn->prepare(
    "SELECT id, journal_name, journal_issn, journal_link, notes,
            status, result_notes, result_file_name, result_file_path, result_file_size, created_at
     FROM journal_evaluation_requests WHERE user_id = ? ORDER BY created_at DESC"
);
...
while ($row = $res->fetch_assoc()) {
    $requests[] = [
        ...
        'meta_ar' => $row['journal_name'],
        'meta_en' => $row['journal_name'],
