        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 mb-5">
            {[
              ...
              {
                to: "/dashboard/help",
                label: currentLang === "ar" ? "مركز المساعدة" : "Help Center",
              },
              {
                to: "/dashboard/contact-us",
                label: currentLang === "ar" ? "تواصل معنا" : "Contact Us",
              },
              {
                to: "/dashboard/terms",
                label:
              ...
              <Link
                key={link.to}
                to={link.to}
                className="text-[11px] text-gray-500 dark:text-gray-500 hover:text-brand-orange transition-colors duration-200"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 mb-5 text-gray-500 dark:text-gray-500">
            {siteSettings?.phone && (
              <a
                ...
                className="flex items-center gap-1.5 text-[11px] hover:text-brand-orange transition-colors"
                dir="ltr"
              >
            ...
            {siteSettings?.address_ar && (
              <span className="flex items-center gap-1.5 text-[11px]">
                ...
                {currentLang === "ar"
                  ? siteSettings.address_ar
                  : siteSettings.address_en || siteSettings.address_ar}
              </span>
            )}
          </div>
          ...
          <div className="border-t border-gray-200 dark:border-brand-dark-border/30 pt-4 flex flex-col sm:flex-row items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              {siteSettings?.site_logo ? (
                <img
                  src={resolveUploadUrl(siteSettings.site_logo)}
                  alt="Logo"
                  className="h-5 w-auto max-w-[20px] object-contain rounded opacity-40"
                />
              ) : (
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-gray-400 dark:text-gray-600">
              ...
              <span>
                {currentLang === "ar" ? "محمي بتشفير SSL" : "Secured by SSL"}
              </span>
            </div>
          </div>
        </div>
