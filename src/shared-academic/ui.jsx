...
export const CardHeader = ({ title, subtitle, actions, className = "" }) => (
  <div
    className={`flex flex-wrap items-center justify-between gap-3 mb-5 ${className}`}
  >
    <div>
      <h2 className="text-gray-900 dark:text-white text-lg font-black leading-tight">
        {title}
      </h2>
      {subtitle && (
...
export const StatusPill = ({ tone = "gray", children, className = "" }) => (
  <span
    className={`inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold whitespace-nowrap ${PILL_TONES[tone] || PILL_TONES.gray} ${className}`}
  >
    {children}
  </span>
);
...
export const KpiCard = ({
  ...
  const body = (
    <Card
      className="p-6 h-full hover:-translate-y-0.5 transition-transform duration-200"
    ...
    >
      ...
      <p
        className="text-gray-900 dark:text-white text-3xl lg:text-[34px] font-black leading-none mb-2"
        dir="ltr"
      >
...
export const Avatar = ({ name, size = "md", className = "" }) => {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded-full bg-brand-orange text-white font-black flex-shrink-0 ${sizes[size] || sizes.md} ${className}`}
    >
      {initial}
    </span>
  );
...
export const PageTitle = ({ title, subtitle, actions }) => (
  <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
    <div>
      <h1 className="text-gray-900 dark:text-white text-2xl lg:text-3xl font-black leading-tight">
        {title}
      </h1>
      {subtitle && (
