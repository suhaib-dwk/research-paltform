...
const CRITICAL_ALERTS = [
  {
    id: 1,
    type: "critical",
    title: "Energy Sector Gap",
    desc: "Research output is 40% below national targets.",
    time: "2h ago",
  },
  {
    id: 2,
    type: "warning",
    title: "Funding Delay",
    desc: "Disbursement for Q3 grants pending.",
    time: "5h ago",
  },
];
...
const StatCard = ({ title, value, icon: Icon, trend, trendValue, color }) => (
  ...
  >
    <div className="flex items-start justify-between mb-4">
      ...
      {trend && (
        <span
          className={`flex items-center text-xs font-bold px-2 py-1 rounded-full ${trend > 0 ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-600"}`}
        >
          {trend > 0 ? (
...
const ChartCard = ({ title, children, className = "" }) => (
  <div
    className={`bg-white dark:bg-[#1a1613]/60 border border-gray-100 dark:border-[#3a322c]/40 rounded-2xl p-6 shadow-sm ${className}`}
  >
...
const AlertCard = ({ alert }) => {
  ...
  return (
    <div
      className={`p-4 rounded-xl border ${isCritical ? "bg-red-50 border-red-100 dark:bg-red-900/20 dark:border-red-800" : "bg-amber-50 border-amber-100 dark:bg-amber-900/20 dark:border-amber-800"}`}
    >
      <div className="flex items-start gap-3">
        <div
          className={`p-2 rounded-lg ${isCritical ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-600"}`}
        >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white flex items-center gap-3">
            ...
            {isAr
              ? "لوحة الذكاء البحثي الوطني"
              : "National Research Intelligence"}
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {isAr
              ? "نظرة شاملة على حالة البحث العلمي، الأولويات، وأداء القطاعات"
              : "Overview of research status, priorities, and sector performance"}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            <RefreshCw
              className={`w-5 h-5 ${refreshing ? "animate-spin text-[#e8623a]" : ""}`}
            />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <ChartCard
            title={
              isAr
                ? "الاتجاه الوطني (النشر والتمويل)"
                : "National Trend (Pubs & Funding)"
            }
          >
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={NATIONAL_TREND_DATA}
                ...
                >
                  ...
                  <Area
                    ...
                    fillOpacity={1}
                    fill="url(#colorPubs)"
                    name={isAr ? "منشورات" : "Publications"}
                  />
                  <Line
                    yAxisId="right"
                    type="monotone"
          ...
          <ChartCard
            title={
              isAr
                ? "توزيع البحث العلمي (حسب المجال)"
                : "Research Distribution by Field"
            }
          >
          ...
          <ChartCard
            title={
              isAr ? "أداء المناطق (Top 5)" : "Regional Performance (Top 5)"
            }
          >
            <div className="space-y-3">
              {REGIONAL_DATA.map((region, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="w-24 text-xs font-bold text-gray-500">
                    {region.region}
                  </div>
                  ...
                  <div className="w-12 text-right text-xs font-bold text-gray-900 dark:text-white">
                    {region.pubs}
                  </div>
          ...
          <div className="bg-white dark:bg-[#1a1613]/60 border border-gray-100 dark:border-[#3a322c]/40 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-gray-800 dark:text-white">
                {isAr ? "تغطية الأولويات" : "Priority Coverage"}
              </h3>
              ...
              {PRIORITY_STATUS.map((item, i) => (
                <div key={i} className="group">
                  <div className="flex justify-between text-xs mb-1.5">
                    ...
                    <span
                      className={`font-bold ${item.status === "Critical" ? "text-red-500" : item.status === "High" ? "text-amber-500" : "text-emerald-500"}`}
                    >
                      {item.coverage}%
                    </span>
                  </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center z-10">
                ...
                <h3 className="text-white font-bold text-lg">
                  {isAr ? "الخريطة البحثية" : "Research Map"}
                </h3>
                <p className="text-gray-400 text-xs mt-1 px-4">
                  {isAr
                    ? "تحليل مكاني للنشاط البحثي"
                    : "Spatial Analysis of Activity"}
                </p>
