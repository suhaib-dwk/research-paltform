  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-center gap-2 mb-4 flex-wrap text-xs">
            <Link to="/dashboard" className="text-gray-400 hover:text-[#c8a44e]">{isAr ? 'الرئيسية' : 'Home'}</Link>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-black text-[#0a1628] mb-2">{title}</h1>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl border border-gray-100 p-6">
        <h2 className="text-lg font-black text-[#0a1628] mb-5 flex items-center gap-2">
          ...
          {STEPS.map((step, i) => (
            <div key={i} className="flex gap-3 p-4 rounded-xl border border-gray-100 hover:border-emerald-200 transition-colors">
              ...
              <div>
                ...
                <h3 className="text-sm font-bold text-[#0a1628] mb-0.5">{isAr ? step.title_ar : step.title_en}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required placeholder={isAr ? 'مثال: ذكاء اصطناعي - تعلم عميق' : 'e.g., AI - Deep Learning'} className="w-full bg-[#f4f6fb] border border-gray-100 rounded-xl px-4 py-3 text-sm text-[#0a1628] outline-none focus:border-emerald-500 transition-colors" />
            </div>
            <div>
              ...
              <input type="text" placeholder="e.g., IEEE Access" dir="ltr" className="w-full bg-[#f4f6fb] border border-gray-100 rounded-xl px-4 py-3 text-sm text-[#0a1628] outline-none focus:border-emerald-500 transition-colors" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea rows={3} placeholder={isAr ? 'أضف أي ملاحظات...' : 'Add any notes...'} className="w-full bg-[#f4f6fb] border border-gray-100 rounded-xl px-4 py-3 text-sm text-[#0a1628] outline-none focus:border-emerald-500 transition-colors resize-none" />
          </div>
          ...
          {FAQS.map((faq, i) => (
            <div key={i} className={`border rounded-xl overflow-hidden transition-colors ${openFaq === i ? 'border-emerald-200 bg-emerald-50/30' : 'border-gray-100'}`}>
              <button type="button" onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between gap-3 p-4 text-start">
                <span className="text-sm font-bold text-[#0a1628]">{isAr ? faq.q_ar : faq.q_en}</span>
