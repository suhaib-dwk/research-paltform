      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white dark:bg-[#211c18] rounded-2xl border border-gray-100 dark:border-[#3a322c]/50 p-5 hover:shadow-md transition-shadow">
                ...
                {!item.is_outgoing && item.status === 'pending' && (
                  <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-[#3a322c]/50">
                    <button
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-white dark:bg-[#211c18] rounded-2xl border border-gray-100 dark:border-[#3a322c]/60 shadow-xl max-h-[85vh] flex flex-col"
            >
              <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-[#3a322c]/50 flex-shrink-0">
                <h2 className="font-black text-gray-900 dark:text-white">{isAr ? 'طلب تعاون جديد' : 'New Collaboration Request'}</h2>
                <button onClick={() => setShowModal(false)} className="p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-[#2a231e] rounded-lg transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
                        ...
                        {partnersLoading ? (
                          ...
                          filteredPartners.map((p) => (
                            <button
                              ...
                              className="w-full flex items-center justify-between text-start px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-[#2a231e] transition-colors"
                            >
                              <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{p.name}</span>
                              <span className="text-[10px] font-bold text-gray-400 bg-gray-100 dark:bg-[#2a231e] px-2 py-1 rounded-full">{roleLabel(p.role, isAr)}</span>
                            </button>
                          ))
                        )}
                      </div>
              ...
              <div className="p-5 border-t border-gray-100 dark:border-[#3a322c]/50 flex-shrink-0">
                <button
