...
const ServicePageWrapper = ({
  ...
  const DefaultRequestCard = ({ req }) => {
    ...
    return (
      <div className="bg-white dark:bg-[#0c1425] rounded-2xl border border-gray-200 dark:border-[#1e3050]/50 p-5 hover:border-[#c8a44e]/20 dark:hover:border-[#c8a44e]/15 transition-all">
          ...
          <div className="flex items-center gap-2 sm:flex-shrink-0">
            {req.score != null && (
              <div className="text-center px-2">
                <div className="text-lg font-black text-[#c8a44e]">{req.score}%</div>
        ...
        {req.status === 'needs_info' && req.info_needed_ar && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#1e3050]/30">
            <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-900/15 border border-amber-200 dark:border-amber-800/30">
        ...
        {/* شريط تقدم لقيد التنفيذ */}
                ...
                {req.status === 'in_progress' && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#1e3050]/30">
                        ...
                        <div className="w-full h-1.5 bg-gray-100 dark:bg-[#1e3050] rounded-full overflow-hidden">
                            <motion.div
                                initial={{ width: 0 }}
                ...
                {req.status === 'completed' && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#1e3050]/30 space-y-3">
                        {/* ملاحظات المحكم */}
                        {req.reviewer_notes && (
                        ...
                        {/* مرفق الملف بعد التحكيم + زر التنزيل */}
                        {req.file_path && (
                            <div className="flex items-center gap-2 flex-wrap">
                                <a
                                    href={`${API_BASE_URL}/${req.file_path}`}
                                    download={req.file_name}
                                    className="flex items-center gap-1.5 text-xs font-semibold text-[#c8a44e] hover:text-[#a8872e] bg-[#c8a44e]/10 hover:bg-[#c8a44e]/20 px-3 py-2 rounded-lg transition-colors"
                                >
                                    <FileText className="w-4 h-4" />
                                    {isAr ? 'تحميل الملف المُراجع' : 'Download Reviewed File'}
                                </a>
                                {req.file_size && (
                                    <span className="text-[10px] text-gray-400">({formatFileSize(req.file_size)})</span>
                                )}
                            </div>
                        )}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ---- شريط التبويبات ---- */}
      <div className="flex gap-1 p-1 bg-gray-100 dark:bg-[#0a1628] rounded-xl overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.key}
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${
              activeTab === tab.key
                ? 'bg-white dark:bg-[#0c1425] text-gray-900 dark:text-white shadow-sm'
          ...
          >
            ...
            {tab.key === 'requests' && mockRequests.length > 0 && (
              <span className={`ms-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                activeTab === tab.key ? 'bg-[#c8a44e]/15 text-[#c8a44e]' : 'bg-gray-200 dark:bg-[#1e3050] text-gray-500 dark:text-gray-400'
              }`}>
                {mockRequests.length}
              </span>
            )}
      ...
      {activeTab === 'guide' && (
        <div className="space-y-3">
          {guideSections.length === 0 ? (
            <div className="bg-white dark:bg-[#0c1425] rounded-2xl border border-gray-200 dark:border-[#1e3050]/50 p-10 text-center">
            ...
            guideSections.map(section => {
              ...
              return (
                <div key={section.id} className="bg-white dark:bg-[#0c1425] rounded-2xl border border-gray-200 dark:border-[#1e3050]/50 overflow-hidden">
                  <button
                    onClick={() => toggleSection(section.id)}
                    className="w-full flex items-center justify-between p-4 sm:p-5 hover:bg-gray-50 dark:hover:bg-[#1a2744]/30 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-gray-100 dark:bg-[#1a2744] rounded-lg flex items-center justify-center flex-shrink-0">
                    ...
                    {isOpen && (
                      ...
                      >
                        <div className="px-5 pb-5 pt-0">
                          <div className="border-t border-gray-100 dark:border-[#1e3050]/30 pt-4">
                            {/* قائمة عناصر */}
                            {section.items_ar && (
        ...
        <div className="space-y-4">
          ...
          {loadingRequests ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <div className="w-8 h-8 border-2 border-[#c8a44e]/30 border-t-[#c8a44e] rounded-full animate-spin" />
            ...
            <div className="bg-white dark:bg-[#0c1425] rounded-2xl border border-gray-200 dark:border-[#1e3050]/50 p-12 text-center">
              ...
              <button
                ...
                className="px-4 py-2 bg-[#c8a44e]/10 text-[#c8a44e] text-sm font-semibold rounded-xl hover:bg-[#c8a44e]/20 transition-colors"
              >
