...
const HelpDashboardPage = () => {
  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-2xl font-black text-[#1a1613]">{t('help.title')}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-gray-100 p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
          <div className="w-11 h-11 rounded-xl bg-[#e8623a]/10 text-[#e8623a] flex items-center justify-center flex-shrink-0"><BookOpen className="w-5 h-5" /></div>
          <div><h3 className="font-bold text-[#1a1613] text-sm">{t('help.guide')}</h3><p className="text-xs text-gray-400 mt-1">{t('help.guide_desc')}</p></div>
        </div>
        <Link to="/contact-us" className="bg-white rounded-2xl border border-gray-100 p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
          ...
          <div><h3 className="font-bold text-[#1a1613] text-sm">{t('help.contact')}</h3><p className="text-xs text-gray-400 mt-1">{t('help.contact_desc')}</p></div>
        </Link>
        <Link to="/help" className="bg-white rounded-2xl border border-gray-100 p-5 flex items-start gap-4 hover:shadow-md transition-shadow">
          ...
          <div><h3 className="font-bold text-[#1a1613] text-sm">{t('help.faq_page')}</h3><p className="text-xs text-gray-400 mt-1">{t('help.faq_page_desc')}</p></div>
        </Link>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <h2 className="text-lg font-bold text-[#1a1613] mb-6">{t('help.faq_title')}</h2>
        <div className="space-y-4">
          {faqItems.map((item, i) => (
            <details key={i} className="group border border-gray-100 rounded-xl overflow-hidden">
              <summary className="flex items-center justify-between p-4 cursor-pointer hover:bg-[#f4f6fb] transition-colors text-sm font-semibold text-[#1a1613]">
