...
// =========================================================
// المساعد الذكي — واجهة محادثة بأسلوب الأنظمة العالمية (ChatGPT / Claude):
//   • حالة بداية مركزية: تحية + بطاقات اقتراحات جاهزة.
//   • عمود رسائل مركزي واسع: ردود المساعد بلا فقاعة (نص على الخلفية) مع
//     زر نسخ، ورسائل المستخدم في فقاعة ناعمة جهة النهاية.
//   • مؤشر كتابة (ثلاث نقاط)، ومؤلِّف لاصق أسفل الشاشة بحقل يتمدد تلقائيًا
//     (Enter للإرسال، Shift+Enter لسطر جديد) وتنويه أن المساعد قد يخطئ.
//   • دليل الاستخدام في لوحة جانبية منزلقة بدل تبويبات الصفحة.
// منطق الاتصال بـ ai_chat_assistant.php كما هو دون تغيير.
// =========================================================
...
// ✅ اقتراحات البداية (تملأ حقل الإدخال ببداية الطلب لتكملته)
const QUICK = [
  { icon: Lightbulb, ar: 'اقتراح عنوان بحثي', en: 'Suggest a research title', d_ar: 'خمسة عناوين محكمة في مجالك', d_en: 'Five focused titles in your field', p_ar: 'اقترح 5 عناوين بحثية في مجال ', p_en: 'Suggest 5 research titles in ' },
  { icon: BookOpenText, ar: 'كتابة ملخص', en: 'Write an abstract', d_ar: 'ملخص أكاديمي بحدود 250 كلمة', d_en: 'An academic abstract of ~250 words', p_ar: 'ساعدني في كتابة ملخص بحث عن ', p_en: 'Help me write a research abstract about ' },
  { icon: PenLine, ar: 'تحسين نص', en: 'Improve text', d_ar: 'صياغة أوضح وأكثر أكاديمية', d_en: 'Clearer, more academic wording', p_ar: 'حسّن النص التالي أكاديمياً: ', p_en: 'Improve the following text academically: ' },
  { icon: FlaskConical, ar: 'مراجعة منهجية', en: 'Review methodology', d_ar: 'اتساق المشكلة والأهداف والأدوات', d_en: 'Fit of problem, objectives and tools', p_ar: 'راجع اتساق منهجية بحثي التالية: ', p_en: 'Review the consistency of the following methodology: ' },
];
...
const AiAssistantPage = () => {
  ...
  // ✅ حقل إدخال يتمدد تلقائيًا حتى ~6 أسطر
  useEffect(() => {
  ...
  // و lang المُرسل للخادم يعتمد على لغة رسالة المستخدم الحالية فعلياً، لا على
  // currentLang العامة المطبَّقة بأثر رجعي على كل التاريخ.
  const handleSend = async (text) => {
    const content = (text ?? input).trim();
    if (!content || isLoading || !entityId) return;
    const userMessage = { id: Date.now(), role: 'user', content, lang: currentLang };
  ...
  const guideSections = [
    ...
    { id: 'tips', icon: Lightbulb, title_ar: 'نصائح لنتائج أفضل', title_en: 'Tips for Better Results', items_ar: ['كن محدداً في سؤالك', 'أذكر التخصص والمجال', 'اطلب التنسيق أو الصياغة بشكل صريح', 'استخدم الاقتراحات كنقطة بداية'], items_en: ['Be specific', 'Mention your field', 'Explicitly request formatting', 'Use suggestions as a starting point'] },
  ...
  return (
    // ✅ هوامش سالبة تلغي حشوة <main> ليملأ المحادثة كامل المساحة تحت الهيدر (5rem)
    <div className="-m-4 sm:-m-6 lg:-m-8 lg:h-[calc(100vh-5rem)] flex flex-col">
      <div className="flex-1 flex min-h-0 relative">

        {/* ─── عمود المحادثة ─── */}
        <div className="flex-1 flex flex-col min-w-0 min-h-[calc(100vh-5rem)] lg:min-h-0">

          {/* رأس المحادثة */}
          <div className="flex items-center justify-between gap-3 px-4 sm:px-6 lg:px-8 py-4 bg-white/80 dark:bg-brand-dark/80 backdrop-blur border-b border-gray-100 dark:border-brand-dark-border/60">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-gray-900 dark:text-white text-base font-black leading-tight truncate">{isAr ? 'المساعد الذكي' : 'AI Assistant'}</h1>
                <p className="text-[11px] text-gray-400 dark:text-gray-500 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                  {isAr ? 'مساعد للأبحاث الأكاديمية — القرار الأكاديمي النهائي لك' : 'Academic research assistant — the final academic decision is yours'}
                </p>
              </div>
            </div>
              ...
              {isFresh ? (
                <div className="flex flex-col items-center text-center pt-6 sm:pt-14">
                  ...
                  <h2 className="text-gray-900 dark:text-white text-2xl sm:text-3xl font-black mb-2">
                    {isAr ? `${firstName ? `مرحباً ${firstName}، ` : ''}كيف يمكنني مساعدتك اليوم؟` : `${firstName ? `Hi ${firstName}, ` : ''}how can I help you today?`}
                  </h2>
                  <p className="text-gray-500 dark:text-gray-400 text-sm max-w-md mb-8">
                    {isAr ? 'اختر اقتراحًا للبدء أو اكتب طلبك مباشرة — أذكر تخصصك للحصول على نتائج أدق.' : 'Pick a suggestion to start or type your request — mention your field for sharper results.'}
                  </p>
                  <div className="grid sm:grid-cols-2 gap-3 w-full">
                    {QUICK.map((q) => (
                      <button
                        key={q.en}
                        type="button"
                        onClick={() => { setInput(isAr ? q.p_ar : q.p_en); textareaRef.current?.focus(); }}
                      ...
                      >
                        ...
                        <span className="min-w-0">
                          <span className="block text-sm font-bold text-gray-900 dark:text-white">{isAr ? q.ar : q.en}</span>
                          <span className="block text-xs text-gray-400 dark:text-gray-500 mt-0.5">{isAr ? q.d_ar : q.d_en}</span>
                        </span>
                      </button>
                    ))}
                  </div>
                    ...
                    m.role === 'user' ? (
                      <div key={m.id} className="flex justify-end">
                        <div className="flex items-end gap-2.5 max-w-[85%] flex-row-reverse">
                          ...
                          <div dir={m.lang === 'ar' ? 'rtl' : m.lang ? 'ltr' : undefined} className="px-4 py-3 rounded-3xl rounded-ee-md bg-brand-orange/10 text-gray-900 dark:text-white text-[15px] leading-relaxed whitespace-pre-wrap">
                            {m.content}
                          </div>
                        </div>
                      </div>
                      ...
                      <div key={m.id} className="group flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div dir={m.lang === 'ar' ? 'rtl' : m.lang ? 'ltr' : undefined} className="text-gray-800 dark:text-gray-100 text-[15px] leading-[1.9] whitespace-pre-wrap">
                            {m.content}
                          </div>
                          {m.id !== 1 && (
                            <div className="mt-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                              ...
                              >
                                ...
                                {copiedId === m.id ? (isAr ? 'تم النسخ' : 'Copied') : (isAr ? 'نسخ' : 'Copy')}
                              </button>
                            </div>
                          )}
                        </div>
          ...
          <div className="px-4 sm:px-6 lg:px-8 pb-4 sm:pb-6 pt-2">
            <div className="max-w-3xl mx-auto">
              {errorNotice && (
                <div className={`flex items-start gap-2.5 p-3 mb-3 rounded-2xl text-sm font-medium ${
                  ...
                  <button type="button" onClick={() => setErrorNotice(null)} className="opacity-60 hover:opacity-100" aria-label="close"><X className="w-4 h-4" /></button>
                </div>
              )}
              {!isFresh && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {QUICK.slice(0, 3).map((q) => (
                    <button key={q.en} type="button" onClick={() => { setInput(isAr ? q.p_ar : q.p_en); textareaRef.current?.focus(); }} className="px-3 py-1.5 rounded-full bg-white dark:bg-brand-dark-card shadow-sm text-gray-600 dark:text-gray-300 text-xs font-semibold hover:text-brand-orange hover:shadow-md transition-all">
                      {isAr ? q.ar : q.en}
                    </button>
                  ))}
                </div>
              ...
              <div className="rounded-3xl bg-white dark:bg-brand-dark-card shadow-[0_16px_48px_-18px_rgba(31,26,23,0.28)] dark:shadow-none dark:border dark:border-brand-dark-border/60 p-2 ps-4 flex items-end gap-2 focus-within:ring-2 focus-within:ring-brand-orange/30 transition-shadow">
                <textarea
                  ...
                  rows={1}
                  placeholder={isAr ? 'اكتب طلبك… (Shift + Enter لسطر جديد)' : 'Type your request… (Shift + Enter for a new line)'}
                ...
                >
                  {isLoading ? <span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : <ArrowUp className="w-5 h-5" />}
                </button>
              </div>
              <p className="text-center text-[11px] text-gray-400 dark:text-gray-500 mt-2.5">
                {isAr ? 'قد يُخطئ المساعد — تحقّق من المعلومات والمراجع المقترحة قبل الاعتماد عليها.' : 'The assistant can make mistakes — verify information and suggested references before relying on them.'}
              </p>
            </div>
          </div>
        </div>
            ...
            <>
              ...
              >
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-gray-900 dark:text-white text-sm font-black">{isAr ? 'دليل الاستخدام' : 'Usage guide'}</h2>
                  <button type="button" onClick={() => setGuideOpen(false)} className="w-8 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-brand-dark-hover flex items-center justify-center text-gray-500" aria-label="close">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                  ...
                  {guideSections.map((section) => (
                    <div key={section.id}>
                      <div className="flex items-center gap-2 mb-2.5">
                        <span className="w-7 h-7 rounded-lg bg-brand-orange/10 text-brand-orange flex items-center justify-center"><section.icon className="w-3.5 h-3.5" /></span>
                        <h3 className="text-[13px] font-bold text-gray-900 dark:text-white">{isAr ? section.title_ar : section.title_en}</h3>
                      </div>
                      <ul className="space-y-1.5 ps-2">
                        {(isAr ? section.items_ar : section.items_en).map((item) => (
                          <li key={item} className="flex items-start gap-2 text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
